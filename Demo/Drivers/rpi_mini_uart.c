...
If the system clock is 250 MHz and the baud register is zero the baudrate
is 31.25 Mega baud. (25 Mbits/sec or 3.125 Mbytes/sec). The lowest baudrate
with a 250 MHz system clock is 476 Baud.
*/
 ...
 * these two functions are based on the same functions in the linux
 * kernel code in drivers/tty/serial/8250/8250_port.c
 */

// before calling this function the caller should first check if the
// condition (tx_to_write > 0 &&  (GET32(AUX_MU_LSR_REG) & UART_LSR_THRE)) is true
...
// before calling this function the caller should first check if the
// condition status & (UART_LSR_DR | UART_LSR_BI) is true
...
void rpi_uart_init ( void )
{
	...
	rpi_gpio_sel_fun(14, 0);
	rpi_gpio_sel_fun(14, 2);
	rpi_gpio_set_conf(14, 0);

	rpi_gpio_sel_fun(15, 0);
	rpi_gpio_sel_fun(15, 2);
	rpi_gpio_set_conf(15, 2);
